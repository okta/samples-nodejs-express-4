...
handlers.profile = (req, res) => {
  ...
  res.render('profile', { user: req.user, config });
};
...
/**
 * Login handler that initiates the OpenID Connect authorization code flow:
 * 1. Sets state and nonce
 * 2. Constructs the /authorize url with the correct query params, and redirects
 *
 * Note: This route accepts a ?sessionToken=foo query parameter - this is used
 * in the custom login flow after completing authentication via the Authn api.
 */
handlers.login = (passport) => passport.authenticate('okta');
// handlers.login = (passport) => {
//   return (req, res) => {
//     res.end('');
//   };
// };
...
/**
 * Callback redirect handler for the OpenID Connect authorization code flow:
 * 1. Exchanges the returned authorization code for an id_token
 * 2. Validates the JWT by verifying the signature and claims
 * 3. If successful, sets the session with the user and redirects to profile
 */
handlers.callback = (passport) => (
  function (req, res) {
    res.status(200).send({});
  }
  // passport.authenticate('okta', {successRedirect: '/authorization-code/profile'})
);

/**
 * Callback redirect handler for the OpenID Connect flow:
