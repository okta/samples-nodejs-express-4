...
const keysGood = require('../lib/keys-good');
const keysBad = require('../lib/keys-bad');
...
function createIdToken(options, kid) {
  options || (options = {});
  const jwsOptions = {
    header: {
      alg: 'RS256',
...
function mockOktaRequests(options) {
  ...
  if (!options.cachedKeyRequest) {
    const req = { url: '/oauth2/v1/keys' };
    const res = { keys: [options.publicJwk || keysGood.publicJwk] };
    res.keys[0].kid = kid;
    reqs.push({ req, res, optional: options.keysOptional });
  }
    ...
    describe('Validating /oauth2/v1/token response', () => {
      ...
      describe('Signature', () => {
        ...
        it('returns 401 if id_token is signed with an invalid cert', () => {
          const mock = util.expand('idToken.secret', keysBad.privatePem);
          const req = mockOktaRequests(mock).then(validateCallback);
          return util.should401(req, errors.CODE_TOKEN_INVALID_SIG);
        });
        ...
        it('caches responses to /oauth2/v1/keys', () => {
          ...
          const withKid2 = () => {
            const mock = {
              kid: kid2,
              idToken: {
                secret: keysBad.privatePem
              },
              publicJwk: keysBad.publicJwk
            };
