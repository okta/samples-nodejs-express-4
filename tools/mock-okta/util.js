 ...
 * @arg {boolean} - flag to indicate if mock server is running in record mode
...
util.mapRequestToCache = (req, record) => {
  ...
  if (req.url.indexOf('authorize?') > -1) {
    ...
    // If playing back the tapes, replace the state in req.url with a known state
    if (!record) {
      req.url = req.url
      .replace(`state=${query.state}`, `state=${config.state}`)
...
util.mapCachedHeadersToResponse = (headers, data, record) => {
  ...
  if (data.isRedirectCallback && !record) {
    mapped.location = mapped.location.replace(`state=${config.state}`, `state=${data.state}`);   
  }
...
util.mapCachedBodyToResponse = (chunk, data) => {
  ...
  if (data.isAuthorizeReq && data.responseMode === 'okta_post_message') {
    ...
    // While playing back the tapes, we need to replace the state in the body with a 'known' state 
    if (!record) {
      newChunk = newChunk.replace(`${config.state}`, data.state)
    }
  }
